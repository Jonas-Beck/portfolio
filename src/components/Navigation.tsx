...
export const Navigation: React.FC<NavigationProps> = ({ sections }) => {
    ...
    const options: IntersectionObserverInit = {
        rootMargin: "0px 0px -60% 0px", // Add negative margin on bottom to focus on the section currently displayed near the top
        threshold: 0.7, // Number to indicate at what percentage of the targets visibility the observer's callback should be executed
    }
    ...
    return (
        <nav className="flex flex-col mt-16 navbar">
            {sections.map((section) => (
                <a href={`#${section.id}`} className={`${section.id == currentSectionId ? "text-2xl transition-all font-bold" : "transition-all hover:text-2xl"}`}>{section.name}</a>
            ))}
        </nav>
    )
}
